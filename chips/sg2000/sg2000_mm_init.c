/****************************************************************************
 * vendor/sg2000/chips/sg2000/sg2000_mm_init.c
 *
...
#define MMU_IO_SIZE      (0x80000000ul)  /* 2 GiB: 0x00000000-0x7FFFFFFF
                                          * Covers all peripherals incl. GMAC
                                          * at 0x04070000 and PLIC at 0x70000000
                                          */

#define MMU_INT_BASE     (0x70000000ul)  /* PLIC — now covered by MMU_IO region */
#define MMU_INT_SIZE     (0x10000000ul)

#define MMU_DDR_BASE     (0x80000000ul)
#define MMU_DDR_SIZE     (0x40000000ul)
...
void sg2000_kernel_mappings(void)
{
  ...
  /* Map DDR region (kernel code, data, heap, ramdisk live here) */

  binfo("map DDR region\n");
  mmu_ln_map_region(1, PGT_L1_VBASE, MMU_DDR_BASE, MMU_DDR_BASE,
                    MMU_DDR_SIZE, MMU_KDATA_FLAGS | MMU_THEAD_PMA_FLAGS);

  /* PLIC at 0x70000000 is now covered by the expanded I/O L1 mapping above.
   * No separate L2 mapping needed.
   */
